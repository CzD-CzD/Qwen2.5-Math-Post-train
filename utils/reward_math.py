...
def are_equal_under_sympy(ground_truth_normalized: str, given_normalized: str):
    ...
    try:
        ...
        pass
    try:
        a = sympy.N(_sympy_parse(ground_truth_normalized))
        b = sympy.N(_sympy_parse(given_normalized))
        if numeric_equal(float(a), float(b)):
            return True
    except Exception:
        pass
    return are_equal
...
if __name__ == "__main__":
    examples = [
        ...
        ("\\begin{bmatrix}1&0\\\\0&1\\end{bmatrix}", "\\begin{pmatrix}1&0\\\\0&1\\end{pmatrix}"),
    ]
    for pred, gold in examples:
        ok_fast = grade(pred, gold, fast=True)
        #ok_full = grade(pred, gold, fast=False)
        print(f"pred={pred!r} gold={gold!r} fast={ok_fast}")
from __future__ import annotations
