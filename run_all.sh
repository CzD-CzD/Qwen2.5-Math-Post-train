...
# 0) Base
INFER_OUTPUT_NAME="base_infer.jsonl" ${PY} run_experiments.py --task infer --infer-mode basic --mode sft --model ${MODEL} --dataset ${DATASET} \
  > "${LOG_DIR}/base_infer.log" 2>&1
...
# 3) Prefix
#${PY} run_experiments.py --task train --mode prefix --model ${MODEL} --dataset ${DATASET} \
#  > "${LOG_DIR}/prefix.log" 2>&1
# infer (Prefix adapter path)
#INFER_OUTPUT_NAME="prefix_infer.jsonl" ${PY} run_experiments.py --task infer --mode prefix --infer-mode basic --model ${MODEL} --dataset ${DATASET} \
#  --model-path ./out/prefi_math \
#  > "${LOG_DIR}/prefix_infer.log" 2>&1
